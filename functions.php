...
// Zoekpagina - action_search.php
	...
	// Sorteer functies
		...
		function SorteerProductenAZ(){
			...
			// Zoek in de database naar producten die de zoekterm in de naam hebben
			$searchQuery = "SELECT StockItemName, StockItemID, MarketingComments, UnitPrice FROM stockitems WHERE SearchDetails LIKE '%$searchID%' GROUP BY StockItemID ORDER BY StockItemName ASC ";
			...
			// Check of er data beschikbaar is:
			if (mysqli_num_rows($resultSearch) > 0) {
				echo "<div class='search'><h2>Resultaten voor \"$searchID\" gesorteerd op naam (A-Z)</h2></div>";
		...
		function SorteerProductenZA(){
			...
			// Check of er data beschikbaar is:
			if (mysqli_num_rows($resultSearch) > 0) {
				echo "<div class='search'><h2>Resultaten voor \"$searchID\" gesorteerd op naam (Z-A)</h2></div>";
					echo "<div class=\"grid-container-seach\">";
					// Voor elk gevangen resultaat een productweergave printen
					while($row = mysqli_fetch_assoc($resultSearch)) {
						echo "<a href='artikel.php?artikel=".$row['StockItemID']."'>";
		...
		function SorteerProductenASC(){
			// Verbinden met database
			include("connect.php");
			// Verkrijg de zoekterm
			$searchID = filter_input(INPUT_GET, 'search');
			// Zoek in de database naar producten die de zoekterm in de naam hebben
			$searchQuery = "SELECT StockItemName, StockItemID, MarketingComments, UnitPrice FROM stockitems WHERE SearchDetails LIKE '%$searchID%' GROUP BY StockItemID ORDER BY UnitPrice ASC ";
			...
			// Check of er data beschikbaar is:
			if (mysqli_num_rows($resultSearch) > 0) {
				echo "<div class='search'><h2>Resultaten voor \"$searchID\" gesorteerd op prijs (laag - hoog)</h2></div>";
					echo "<div class=\"grid-container-seach\">";
					// Voor elk gevangen resultaat een productweergave printen
					while($row = mysqli_fetch_assoc($resultSearch)) {
						echo "<a href='artikel.php?artikel=".$row['StockItemID']."'>";
		...
		function SorteerProductenDESC(){
			// Verbinden met database
            include("connect.php");
            // Verkrijg de zoekterm
            $searchID = filter_input(INPUT_GET, 'search');
            // Zoek in de database naar producten die de zoekterm in de naam hebben
            $searchQuery = "SELECT StockItemName, StockItemID, MarketingComments, UnitPrice FROM stockitems WHERE SearchDetails LIKE '%$searchID%' GROUP BY StockItemID ORDER BY UnitPrice DESC ";
            ...
            // Check of er data beschikbaar is:
            if (mysqli_num_rows($resultSearch) > 0) {
                echo "<div class='search'><h2>Resultaten voor \"$searchID\" gesorteerd op prijs (hoog - laag)</h2></div>";
                    echo "<div class=\"grid-container-seach\">";
                    // Voor elk gevangen resultaat een productweergave printen
                    while($row = mysqli_fetch_assoc($resultSearch)) {
                        echo "<a href='artikel.php?artikel=".$row['StockItemID']."'>";
...
// Artikelpagina - Artikel.php
	...
	function laadProductpagina(){
		...
		// , QuantityOnHand
		if($artikelID != ""){
			$artikelQuery = "SELECT StockItemName, StockItemID, MarketingComments, UnitPrice, QuantityOnHand, IsChillerStock, CustomFields FROM stockitems JOIN stockitemholdings USING(StockItemID) WHERE StockItemID = $artikelID";
